...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize(info = {})
		...
		register_advanced_options([
			...
			OptInt.new('MAXWAIT', [true,
				'Maximum time (seconds) to wait for exploits to setup before moving on, default is 5',
				5
			]),
		], self.class)
	...
	def init_exploit(name, mod = nil, targ = 0)
		...
		if datastore['CREATEFILES']
			...
			#
			# NEinwechter: Added a while loop and MAXWAIT var to allow the exploits
			# more time to setup if needed. This improved the number of exploits being
			# launched on my system from between 9 and 11 to 85 (Bug #4964)
			time = 0
			sleep_time = 0.5
			while framework.jobs[@exploits[name].job_id.to_s].nil? && time <= datastore['MAXWAIT']
				Rex::ThreadSafe.sleep(sleep_time)
